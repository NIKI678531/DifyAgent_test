...
OUTPUT_EXCEL = "OpenRice_前10家测试结果.xlsx"
# =================================================================
...
def get_correct_restaurant_status(shop_name, area, address):
    ...
    try:
        ...
        # 【关键优化】：如果只有 1 个搜索结果，直接返回
        # 因为搜索本身已经按店名进行，不需要再过度匹配
        if len(elements) == 1:
            try:
                element = elements[0]
                ...
                if len(spans) >= 1:
                    ...
                    if target_url:
                        status = "已结业" if is_closed else "营业中"
                        print(f"   ✓ 单个结果直接返回: {status}")
        ...
        for element in elements:
            try:
                ...
                # 计算店名匹配分数和地址匹配分数
                name_match_score = calculate_restaurant_name_match(shop_name, restaurant_name)
                addr_match_score = calculate_address_match(address, restaurant_address)
                    
                print(f"   → 店名匹配 {name_match_score:.1f}% | 地址匹配 {addr_match_score:.1f}%: {restaurant_name} | {restaurant_address[:50]}... | {'已结业' if is_closed else '营业中'}")
                    
                # 【改进逻辑】多个结果时：只要店名或地址有一个合理匹配就接受
                # 店名 > 40% 或 地址 > 40%
                combined_score = max(name_match_score, addr_match_score)
                is_match = (name_match_score > 40) or (addr_match_score > 40)
                    
                if is_match and combined_score > best_match_score:
...
def calculate_restaurant_name_match(search_name, found_name):
    """
    计算店名匹配度（0-100 分）
    """
    ...
    # 提取主要关键词（去掉常见的餐厅词汇）
    common_words = ['restaurant', 'cafe', 'bar', 'food', 'house', 'place', 'kitchen', 'shop', 'limited', 'ltd', '(closed)']
    search_words = set(re.findall(r'\b\w+\b', search)) - set(common_words)
    found_words = set(re.findall(r'\b\w+\b', found)) - set(common_words)
    
    if not search_words or not found_words:
        return 0.0
    
    intersection = len(search_words.intersection(found_words))
    union = len(search_words.union(found_words))
